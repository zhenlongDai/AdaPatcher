...
#test:第二阶段基于crp执行
bash ./script/pipeline/part/recyle_choseCRP.sh "$end_iterations" "test" "$begin_iterations" "$SAVE_STEPS" "$PROMPT_PATTERN" "$DATA_FILE" "$gpu_seq2test" "$DEVPORT" "$test_batch_size" \
"$SECOND_PROMPT_PATTERN" "$second_checkpoint"
