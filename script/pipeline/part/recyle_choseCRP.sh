...
PORT=$8
batch_size=$9
SECOND_PROMPT_PATTERN=${10}
SECOND_CHECKPOINT=${11}
#SECOND_PROMPT_PATTERN="fixbycrp"
#SECOND_CHECKPOINT=26000

# 检查是否所有参数都已提供
if [ -z "$UPCONT" ] || [ -z "$PATTERN" ] || [ -z "$CHECKPOINT" ] || [ -z "$SAVE_STEPS" ] || [ -z "$PROMPT_PATTERN" ] || [ -z "$DATA_FILE" ] || [ -z "$gpu_seq" ] || [ -z "$PORT" ] || [ -z "$SECOND_PROMPT_PATTERN" ] || [ -z "$SECOND_CHECKPOINT" ]; then
...
do
  ...
  CUDA_VISIBLE_DEVICES=$gpu_seq torchrun --master_port=$PORT --nproc_per_node=$gpu_count -m LoraTrainer.eval_Multi \
      ...
      --predict_filePath "./predict_dir/loraWeight/$PROMPT_PATTERN-GEN/$PATTERN-checkpoint-$CHECKPOINT.json" \
      --per_device_eval_batch_size $batch_size \
      --CRPdata_path "./predict_dir/loraWeight/$PROMPT_PATTERN/$PATTERN-checkpoint-$CHECKPOINT.json"\
