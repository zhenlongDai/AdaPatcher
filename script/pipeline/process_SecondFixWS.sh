...
PATTERN="dev" 
SAVE_STEPS=2000
# base_PROMPT_PATTERN="trace_CRFLP"
# base_checkpoint=XXXX
...
SAVE_FILE="fixbycrflpWS"
DATA_FILE="WsCRFLPDataset"
num_train_epochs=12
train_batch_size=4
eval_batch_size=2
test_batch_size=1
...
if [ "$Exec_status" = "true" ]; then
    ...
    bash ./script/pipeline/part/recyle_Execution.sh "$end_iterations" "$PATTERN" "$begin_iterations" "$SAVE_STEPS" "$SAVE_FILE"
fi
